...
class AssetResponseTest {
    ...
    @Test
    @DisplayName("Should evaluate isKeyValid safely even if validKey is null")
    void shouldEvaluateIsKeyValidSafely() {
        AssetResponse validResponse = createAssetResponse(true, null, false);
        AssetResponse invalidResponse = createAssetResponse(false, null, false);
        AssetResponse nullResponse = createAssetResponse(null, null, false);

        assertTrue(validResponse.isKeyValid());
        assertFalse(invalidResponse.isKeyValid());
        assertFalse(nullResponse.isKeyValid());
    }

    @Test
    @DisplayName("Should evaluate isFromCache safely even if fromCache is null")
    void shouldEvaluateIsFromCacheSafely() {
        AssetResponse cacheResponse = createAssetResponse(true, null, true);
        AssetResponse noCacheResponse = createAssetResponse(true, null, false);
        AssetResponse nullResponse = createAssetResponse(true, null, null);

        assertTrue(cacheResponse.isFromCache());
        assertFalse(noCacheResponse.isFromCache());
        assertFalse(nullResponse.isFromCache());
    }
